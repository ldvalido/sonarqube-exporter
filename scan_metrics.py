from prometheus_client import Enum, Gauge, Info, Histogram
...
def get_stat(metrics):
    ...
    for metric in metrics:
        if metric['type'] in ['INT', 'FLOAT', 'PERCENT', 'MILLISEC', 'RATING', 'WORK_DUR','STRING','DATA']:
            g = Gauge(metric['key'], metric['name'], ['project_key', 'domain'])
        elif metric['type'] == 'DISTRIB':
            g = Histogram(metric['key'], metric['name'])
        elif metric['type'] == 'BOOL':
            g = Info(metric['key'], metric['name'])
        elif metric['key'] == 'alert_status':
            g = Enum(metric['key'], metric['name'], ['project_key', 'domain'], states=['ERROR', 'OK'])
        
        else:
            print('metrics is not supported: ' + metric["key"] + ' ' + metric["type"])
        ...
        if len(measures) > 0:
            if 'value' in measures[0]:
                ...
                try:
                    ...
                    print(error)
                    raise error
            if metric['type'] in ['INT', 'FLOAT', 'PERCENT', 'MILLISEC', 'RATING', 'WORK_DUR','STRING']:
                g.labels(
                    project_key=p['key'], 
                ...
                print('metrics is not supported: ' + p["key"] + ' ' + metric["key"] + ' ' + metric["type"])
        else:
            print('component doesnt have metric: ' + p["key"] + ' ' + metric['key'] + ' ' + metric["type"])
